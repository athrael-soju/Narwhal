...
def run(
    ...
    *,
    ...
    if p.returncode in (124, 255) and node.fabric and jump is not None:
        # The management path answered nothing; the fabric still might: a
        # node's management NIC can die while its engine keeps serving over
        # the ULA, and the jump through a healthy peer is the whole fix.

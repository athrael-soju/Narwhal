...
from narwhal.deployment.engine_launch import load_launches
from narwhal.deployment.host_access import SSH, Host, load_hosts, write_private
from narwhal.deployment.launch_engine import ENV_PREFIXES, MANAGED_ENV, validate_runtime
from narwhal.deployment.prepare_host_env import select_values, write_environment
...
def build_records(
    hosts: list[Host], env: dict[str, str], observations: dict, out: Path
) -> tuple[dict, dict, dict, dict]:
    """Bind detected devices and image metadata to environment-selected deployment policy."""
    launches, sources, derived, shapes = {}, {}, {}, set()
...
def discover(env: dict[str, str], out: Path) -> Path:
    ...
    observations = {}
    checkpoint_jobs: dict[tuple[str, str], tuple[Host, Path, list[str]]] = {}
    for host in hosts:
        ...
        for role in host.roles:
            ...
            probe_inputs = {
                **inputs,
                "environment_prefixes": ENV_PREFIXES,
                "managed_environment": sorted(MANAGED_ENV),
            }
            payload = out / f"{role}-inputs.json"
            write_private(payload, json.dumps(probe_inputs).encode())
            observed = json.loads(
                ssh.run(
                    host,
    ...
    def inspect_checkpoint(job: tuple[Host, Path, list[str]]) -> dict:
        host, payload, _ = job
        return json.loads(
            ssh.run(
                host,

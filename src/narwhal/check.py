...
Engines are addressed by their own host and port, never loopback: the reference
fleet runs two distinct ports and five of six nodes share one.
"""
...
async def gate_consume(
    ...
    router can use any of the 30. A stalling pair outside the ring passed the
    gate while it was losing requests under load.
...
def _pairs_of(producers: list[str], consumers: list[str], mesh: bool) -> list[tuple[str, str]]:
    ...
    pinned both shapes reduce exactly to the old all-ids forms, so an
    unpinned fleet's gate is unchanged.
    """
    if mesh:

...
The design and its evidence live in the papers repository
(Price-of-Order, studies/target-state-planning); the
short form: each plan interval, estimate every pool's demand in
...
controllers over one fleet fight - the naive-hybrid arm of the study spent
388 moves demonstrating it.
"""
...
class Planner:
    ...
    def fast_step(self) -> int:
        ...
        holds surplus; shrinking is the plan loop's job alone. The naive
        version of this loop spent 388 moves fighting the planner; the
        ratchets are what make two loops one controller.
        """
        now = self._clock()
    ...
    def _target(self, now: float) -> int | None:
        ...
        # or idle signal is amplified noise; this one hold was worth 16
        # points on the study's stayer trace.
        ...
        # need flips every window, and before the deadband existed every
        # such flip took this act-now path, hunting at ~2 flips/min while
        # real phase shifts (which overshoot any sane deadband) still act
        # immediately. Measured on the release walks.

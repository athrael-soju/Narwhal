...
from narwhal.deployment.discover_deployment import fabric_address
...
from narwhal.observability.make_targets import build_targets
...
def _address(interface: str) -> str:
    ...
    return fabric_address({}, 1, json.loads(result.stdout))
...
def materialize(
    ...
    *,
    ...
    if output.exists():
        raise FileExistsError(f"Narwhal Dev instance already exists: {output}")
    ...
    _check_free_ports(used_ports, address)
    ...
    for index in range(count):
        ...
        fleet["engines"].append(
            {
                "iid": f"n{number}",
                "url": f"http://{address}:{engine_port}",
                "attestation_url": f"http://{address}:{attest_port}/v1/attestation",
                "role": "prefill" if index == 0 else "decode",
                "shared_device": shared,
            }
        )
        selected_launch(launch, name, {})
    router_url = f"http://{address}:{ports['router']}"
    targets = build_targets(fleet, router_url)
    ...
    try:
        ...
        write_private(
            targets_dir / "router.json",
            json.dumps([{"targets": [targets.router]}], indent=2) + "\n",
        )
        write_private(
            targets_dir / "engines.json",
            json.dumps(
                [{"targets": [host], "labels": {"iid": iid}} for iid, host in targets.engines],
                indent=2,
            )
            + "\n",
        )

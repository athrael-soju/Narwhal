...
class Flip:
    ...
    The in-flight counts are part of the record: a flip that strands work
    costs more than the relabel it looks like.
    """
    ...
    decode_inflight: int
    # The role change is a relabel, so the cost is the work it caught still
...
class Thresholds:
    ...
    # time", not on one crossing. The Arrow paper names no count; the default was
    # tuned over {1, 2, 3, 5} consecutive over-threshold ticks.
    sustained_intervals: int = 3
    # An instance that just flipped may not flip again, in either direction,
    # for this long. 0 is the Arrow paper's behaviour.
    ...
    # `sustained_intervals` consecutive monitoring passes. The condition is
    # two-sided because a global spike raises both loads, and a decode-only
    # trigger strips the prefill pool mid-flood. 0 disables the bypass,
    # which is the Arrow paper's behaviour; enabled values sit at or above 1.
...
class GlobalScheduler:
    ...
    def __init__(
        ...
        # Empty means every engine is fair game.
        self.pinned = pinned
        # The prefill pool's floor; 1 is the Arrow paper's never-empty guard.
        self.min_prefill = max(1, min_prefill)
        # Process start counts as the last P->D flip, so the cooldown gates the
        # first P->D like any later one: a pinned arm relies on a large cooldown to
        # be pinned, and an ungated opening flip would undo the pin.
        ...
        # benefit taken regardless of congestion - the selfish-equilibrium
        # play this ablation exists to price. Off is the shipped
    ...
    def health_pass(self) -> None:
        ...
        loop calls this on both controller branches - on a planner-owned
        fleet this pass is the only drift instrument left running.
        """
        if self.health is None:
            return
    ...
    def regime(self) -> str:
        """The study's three load regimes, from live pool loads."""
        lp = self.pool_load(Role.PREFILL)
        ld = self.pool_load(Role.DECODE)
        peak = max(lp, ld)
    ...
    def _note_panic(self, lp: float, ld: float) -> None:
        ...
        A global spike raises both loads and must not count: a one-sided decode
        trigger strips the prefill pool mid-flood under exactly that flood.
        """
        armed = (

...
class EngineSpec:
    ...
    # move it. Off everywhere by default - a fleet that pins nothing
...
class FleetConfig:
    ...
    # needed instances at once (the design study lives in the papers
    # repository under Price-of-Order studies/target-state-planning).
    ...
    # rebalances and wait for plan_confirmations. 0 restores the pre-deadband
    # act-on-any-ceil-crossing behaviour.
    plan_deadband: float = 0.5
    plan_fast_step_s: float = 5.0
    # Telemetry is configuration, not environment: a run's config file is its
    # record (see the experiments ledger), so the W&B destination belongs in
    # it. Empty project means no exporter.
    wandb_project: str = ""
    wandb_run: str = ""
    ...
    # anonymous tenant over the whole pool, exactly the pre-tenancy door.
    tenants: list[TenantSpec] = field(default_factory=list)
    tenant_auth_required: bool = False
    tenant_anonymous_weight: float = 1.0
    # The prefill pool never shrinks below this. 1 is the Arrow paper's behaviour
    # (a pool is never emptied) and the default; 2 keeps a pinned prefill
    # engine from ever carrying the fleet's whole KV egress alone - the
    # ceiling a role-constrained NIC imposes.
    min_prefill: int = 1
    ...
    def validate(self, source: str = "config") -> None:
        ...
        pinned_prefill = [e.iid for e in self.engines if e.pin and e.role is Role.PREFILL]
        if pinned_prefill and self.min_prefill < 2 and len(self.engines) > 1:
            problems.append(
                "a pinned prefill engine with min_prefill 1 can end up carrying the "
                "fleet's whole KV egress alone; set min_prefill to at least 2 "
                f"(pinned: {', '.join(pinned_prefill)})"
            )
        if problems:

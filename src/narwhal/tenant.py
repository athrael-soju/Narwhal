...
class TenantLedger:
    ...
    whole pool and admission is the pool limit alone: the feature is inert
    by default.
    """
    ...
    def _cap(self, name: str) -> int:
        ...
        # (named shares over named-only, anonymous over the full sum) would
        # let the caps oversubscribe the pool, and the pool-total backstop
        # would refuse a trickle that is inside its own promised share.
    ...
    def can_admit(self, tenant: TenantSpec | None) -> bool:
        ...
        - that is what fairness with a reservation costs, and the design
        pays it on purpose.
        """

...
# every fleet. Measured on one fleet's profile: at ISL 12,000 that phase is
# decode-bound at 1.74 req/s against a prefill ceiling of 7.74, so decode binds
# and the optimum never crosses. Short outputs are what make prefill bind there.
#
...
# base rate. The shipped constants come from one measured profile: about
...
# The model is validated against the fleet: at 4.4 req/s the old prefill phase
# asked 140.6 instance-seconds of prefill, which overran 1P5D's 60 and fitted
# 3P3D's 180, and the run missed 138 on TTFT at 1P5D and none at 3P3D.
PHASE_SECONDS = 300.0
SEGMENTS = (
...
def derive_segments(
    ...
    The baked SEGMENTS multipliers encode one measured fleet's knees
    ...
    this derives (19.4, 1.0, 4.4) against the sim-validated (16, 1, 4).
...
# calibration ran. Sizing prompts by characters with a chars-per-token guess
# once ran 2.7x short and made the prefill-heavy phase far lighter than the
# trace asked for; a wrong ratio here silently mis-sizes every prompt.
FILLER = "benchmark "

...
Follow [Deploy a fleet](docs/Deploy.md) from your management workstation. The guide uses the supplied private inventory and access to prepare the remote router and GPU engine hosts, launch vLLM with NIXL, and verify a model completion through Narwhal. Measure the workload through the private SSH route, reconcile the results, then verify supervised restart and readmission.

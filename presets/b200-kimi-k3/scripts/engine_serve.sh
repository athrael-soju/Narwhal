...
# @ sha256:e90e2603b278) and is reproduced here in full.
#
...
# DS layout: VLLM_SSM_CONV_STATE_LAYOUT=DS
...
if [ -z "${UCX_NET_DEVICES:-}" ]; then
  ...
  if [ -n "$UCX_DEVS" ]; then
    ...
    # No active 400G HCA. Fall back to the interface carrying NIXL_HOST_IP so
    # the engine still boots, and say so - this is the slow path, not the
    # intended one.
    say "no active 400G HCA found; falling back to the interface carrying NIXL_HOST_IP"
    UCX_DEFAULT_DEV=$(ip -o addr show to "$NIXL_HOST_IP" 2>/dev/null | awk '{print $2}' | head -1)
    UCX_NET_DEVICES="${UCX_DEFAULT_DEV:?no interface carries NIXL_HOST_IP; set UCX_NET_DEVICES}"
  fi
fi

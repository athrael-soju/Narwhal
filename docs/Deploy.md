...
4. Run the [deployment workload](Measure.md#2-validate-the-deployment-under-load) from the workstation through `$NARWHAL_TRIAL_URL` at two offered rates and attach its artifacts. Record the latency limits and attainment threshold as candidate targets until measured results and the service requirement establish acceptance. Record client CPU and network utilisation alongside throughput to identify saturation of the load generator or SSH path.
...
| [Capacity acceptance](#10-validate-ingress-and-capacity) | Workstation load client, router-host Docker Compose, existing router SSH access, workload, candidate latency/attainment targets and selected trial or service ingress. | Tunnel bind failure: choose a free local port. Service or scrape failure: check the router-host listener and target error. SLO or accounting failure: reconcile client and router records and measure client, tunnel and engine saturation before repeating the affected check. | Workstation access environment and tunnel logs, router fleet and role environment, Compose discovery files, private workload configuration and deployment evidence store. |

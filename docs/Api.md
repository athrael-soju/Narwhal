...
- Failed legs return their true status: `504` for anything time-shaped and `502` for a faulted engine. A streaming request instead gets the failure as a terminal `data: {"error": ...}` frame, whether or not any tokens were sent first.
...
| `admission`                                  | In-flight count against the limit, and the two refusal counters. A rising `rejected` means the pool refused work it had no room for. A rising `refused` means the cost model priced every placement over the TTFT budget before dispatch. Both reach the client as 429.                                                                                                            |

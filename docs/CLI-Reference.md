...
The run aborts if every `--only` value is absent from the configured engines, an engine fails `/health`, `/tokenize` omits a valid `max_model_len`, the live limit leaves too few sweep points, or the representative prefill fit exceeds 20% mean or 50% worst-point error. Actual tokenised input plus requested output must fit before each completion. A successful run ends with `wrote N profile(s) to PATH`.

...
`narwhal-inference` installs the six commands below. Each accepts `-h` and `--help`, and resolves relative paths from the process working directory.
...
| [`narwhal-engine`](cli/Engine.md)   | Prepare and launch checked vLLM engines from Narwhal launch records     |

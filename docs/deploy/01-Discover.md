...
For two to eight CUDA engines sharing one physical GPU, set `NARWHAL_SHARED_GPU_ALLOWANCE` to a fraction above zero and at most one. Set each role's `NARWHAL_NODE_<n>_GPU_IDS` to the same detected GPU index or UUID and `NARWHAL_NODE_<n>_GPU_MEMORY_UTILIZATION` to its vLLM memory fraction. Discovery requires a detected UUID for each selection, checks that all roles resolve to the same physical GPU, and rejects budgets whose sum exceeds the allowance. For example, eight roles may each use `0.1` under an allowance of `0.9`. The allowance leaves capacity outside the declared vLLM budgets; it is a configured limit, not a measurement of free VRAM.

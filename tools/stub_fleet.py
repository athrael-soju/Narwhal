...
def main() -> int:
    ...
    # One process per instance. A shared event loop across the fleet makes the
    # stub, not the scheduler, the bottleneck: six engines on one loop run a
    # 160 ms token interval against the 12 ms the timing model asks for, and
    # every SLO gate then fails against a limit that belongs to the harness.
    procs = []

...
class ConfigTests(unittest.TestCase):
    ...
    def test_removed_fields_are_rejected(self):
        """Fleet schema 1 rejects retired qualification and payload fields."""
        for field, value in (
            ("production", True),
            ("qualification_path", "qualification.json"),
            ("qualification", {"max_decode_fit_mape": 0.05}),
        ):
            with self.subTest(field=field), self.assertRaisesRegex(ValueError, field):
                self.load({**self.raw, field: value})
        with self.assertRaisesRegex(ValueError, "telemetry"):
            self.load(
                {
                    **self.raw,
                    "telemetry": {"journal_payloads": "runs/payloads.jsonl"},
                }
            )
        with self.assertRaisesRegex(ValueError, "tenants"):
            self.load({**self.raw, "tenants": {"names": []}})
        raw = copy.deepcopy(self.raw)
        raw.setdefault("engine", {})["forward_client_credentials"] = True
        with self.assertRaisesRegex(ValueError, "forward_client_credentials"):
            self.load(raw)

...
class CoverageRunnerTests(unittest.TestCase):
    ...
    def test_commands_cover_unit_ha_and_both_restart_policies(self):
        """The coverage inventory matches the four make-test execution paths."""
        cases = commands(Path("/tmp/coverage-fixture"))
        self.assertEqual(
            [name for name, _ in cases],
            ["unit", "ha", "lifecycle-individual", "lifecycle-wave"],
        )
        self.assertIn("whole_wave", cases[3][1])

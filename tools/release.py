...
def candidate(root: Path) -> tuple[str, int] | None:
...
def publish(root: Path, dist: Path) -> None:
    ...
    # Propagate API failures before checking whether the release exists.
    releases = json.loads(
    ...
    if missing and not release["draft"]:
        raise ValueError("Published release is missing expected artifacts")
    for name in missing:

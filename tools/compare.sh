...
# that fails at startup reads like a 0% result (gauntlet, 2026-08-12).
set -u
cd "$(dirname "$0")/.."
V=.venv/bin
...
# Truly static pins every engine: Algorithm 1's inline rescue flip
# ignores thresholds, and the walk's first static cell drifted through
# exactly that hole. The splice's milder load never fired it - luck, not
# a guarantee.
...
# left it while the pinned arms start clean. research/eval §F asks for arms
# aligned on identical windows for a paired comparison, which needs every
# (arm, rate) to begin from the same configured split.
run_arm () {
  local name="$1" cfg="$2"
  : > "$OUT/$name.bench.log"
